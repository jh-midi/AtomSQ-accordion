...
fun makeKey (row:int) (push: pitch) (draw: pitch) :key =
    let val midi_push = ( pitch2midi push) + transpose
	val midi_draw = (pitch2midi draw) + transpose
	fun noteOn note = if row = 1 then  message (0x90,note,!velocity)
			  else  message (0x91,note,!velocity)
	fun noteOff note = if row = 1 then message (0x80,note,!velocity)
			   else message  (0x81,note,!velocity)
    in
    {push =  push,
     push_midi= midi_push,
...
(* init keyboard array with dummy values *)		 
val keyboard : key array =  Array.array (68, makeKey 1 (C,0) (D,0)) (* on atom the keys go from 0 to 67 *)
...
fun fillKeyboard (tuning : row_tunings) =
	...
	while !pos < 14 do (
	    ...
	    in
		Array.update (keyboard, !row2_col, makeKey 2 push2 draw2);
		Array.update (keyboard, !row1_col, makeKey 1 push1 draw1);
		pos := !pos + 1;
		row2_col := !row2_col + 1;
		row1_col := !row1_col + 1
	    end
...
fun  selectControl(buf,status,data1,data2,ts) =
	 ...
	 val buf2 = bufferNew 1
	 val () =  ArraySlice.copy {src= buf,dst=buf2,di=0}
     in
	 ...
	 if isEncoder()  then  (setEncoder data1 data2 ts;() )
	     ...
	     case data1 of 0wx24 => setTuning' si_do
			 ...
			 | _ => (write DIATO_out buf2 1; ())
     else (write DIATO_out buf2 1;())
     end

 fun discri buf =
     let val ts =  ArraySlice.sub (buf,1)
	 val msg = ArraySlice.sub (buf,0) 
  	val status =  messageStatus msg (* to Word8 *)
	val data1 = messageData1 msg
	val data2 = messageData2 msg
